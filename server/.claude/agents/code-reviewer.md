---
name: code-reviewer
description: Use after writing or modifying any route, handler, service, repository, or error path — and whenever asked to review code, audit a PR, verify architecture, check the API/error contract, cache strategy, analytics pipeline, security, JWT, rate limiting, redirect, short-code generation, or database design. Reviews code against the project's locked design docs and reports specific, actionable violations. Read-only — never edits.
...
- All Prisma calls go through a `*.repository.ts` file — route handlers never touch `app.prisma` directly
