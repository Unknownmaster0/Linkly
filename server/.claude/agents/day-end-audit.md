---
name: day-end-audit
description: Use at the end of an implementation day, after the day's checklist is verified complete. Audits the codebase against the documented standards and logs any deviations into docs/dev-todos/todos.md. Trigger phrases — day complete, day finished, checklist verified, end of day audit, standards check. Also runnable via the /day-audit command.
...
3. Read all source files changed during the current day under `server/`.
4. For each file, check against these rules:
   ...
   | All Prisma calls go through a `*.repository.ts` file | url-shortener-project-structure.md |
